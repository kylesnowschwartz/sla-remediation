...
The tracker (`SLA::Tracker`, `lib/sla/tracker.rb`) runs in a background thread started from `config.ru` and polls every open session every 15 s, recording `status`, `status_detail`, `acus_consumed`, `poll_count`, the first pull request's URL and state, and the structured output on the `sessions` row.
A session is judged `settled` once it has stopped working (`waiting_for_user`, `suspended`, `exit`, or `error`) with a structured output or a pull request, and `stalled` once it has stopped with neither; either outcome closes the row and it is never fetched again, while `waiting_for_approval` keeps it open and logs a warning once.
The first time a row has a pull request the notifier comments on the finding's issue (pull request link, session link, due date and whether it is inside or past the SLA window); `pr_notified_at` is written in the same transaction, so the comment is posted exactly once.
The comment needs `SLA_GITHUB_TOKEN`; without it the `Null` notifier is used and nothing is posted. `SLA_TRACKER=off` skips the thread entirely.
`bin/track` (needs `DEVIN_SERVICE_API_KEY_V3` and `DEVIN_ORG_ID`) runs one round and prints the summary (`polled= settled= stalled= notified= errors=`); `--loop` keeps polling in the foreground until INT or TERM.
