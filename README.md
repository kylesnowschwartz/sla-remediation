...
- `repairing` — the open pull request's checks are red on the commit the tracker last sent back to the session (`ci_repairs` above 0), and the due date has not passed; it reads `[CI FAILING]` again once the checks are observed red on a later commit. The detail row shows `ci repairs: N`. A pull request closed without merging is neither `repairing` nor `ci failing`.
