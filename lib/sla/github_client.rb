...
module SLA
  ...
  class GitHubClient
    ...
    def initialize(token: ENV.fetch('SLA_GITHUB_TOKEN'), connection: nil)
    ...
    def file_contents(repo, path, ref:)
      body = request(:get, "/repos/#{repo}/contents/#{path}", ref: ref)
    ...
    def request(method, path, params = nil)
      response = @connection.run_request(method, path, nil, nil) { |req| req.params.update(params) if params }
